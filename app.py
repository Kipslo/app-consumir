            ...
            def commandpage(number):
                ...
                def reviewpage(event = 0):
                    ...
                    def edit(i):
                        ...
                        for k, j in enumerate(self.text):
                            ...
                            if t:
                                page.add(ft.Row(height=5))
                                page.add(ft.Row(controls=[ft.Container(content=ft.Row([ft.Container(width=10), ft.Container(ft.Text(j), expand=True), ft.CupertinoButton(text="DELETAR", on_click=lambda x, y = k:delete(y), width=100)]), expand=True, bgcolor=ft.colors.BLACK12)], height=50))
                    ...
                    if productscategory[0][0] != '':
                        for i in productscategory:
                            if i[1] != "SIZE":
                                ...
                                """, width=150, height=75, on_click=lambda x, y = i[0], z = i[3]: sizepage(y, z)), bgcolor=ft.colors.BLACK12, width=150, height=75))
                ...
                def sizepage(product, prynter):
                    ...
                    for i in sizes:
                        ...
                        {i[1]}""", width=150, height=75, on_click=lambda x, y = product + f" ({i[0]})", z = i[1], a = "SIZE", b = prynter:addproductlist(y, z, a, b)), bgcolor=ft.colors.BLACK12, width=150, height=75))
